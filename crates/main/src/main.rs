...
fn main() -> Result<(), slint::PlatformError> {
    ...
    if monitor_size.width < 1366 {target_w = 960.0; target_h = 540.0}
    ...
    window.on_window_dragged(move |delta_x, delta_y| {
        if let Some(w) = window_weak.upgrade() {
            ...
            w.window().set_position(slint::WindowPosition::Logical(
                slint::LogicalPosition::new(logical_pos.x as f32 + delta_x, logical_pos.y as f32 + delta_y),
            ));
        }
    });
    ...
    window.on_minimize_clicked(move || {
        if let Some(w) = window_weak.upgrade() {w.window().set_minimized(true)}
    });

    let window_weak = window.as_weak();
    window.on_close_clicked(move || {
        if let Some(w) = window_weak.upgrade() {let _ = w.hide();}
    });
