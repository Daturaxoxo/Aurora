...
# Sometimes, I get happy that AI exists. Sometimes I hate it. But I have to admit, it can genuinely save so much time and fix problems that you've been stuck on for hours. -Datura
...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            ...
            if parse_version(local) < parse_version(online):
                self.update_available.emit(local, online)
            else:
                self.up_to_date.emit()
        except Exception:
            logger.warning(f"Update Checker failed:\n{traceback.format_exc()}", extra={"el": True})
...
class UpdateWorker(QThread):
    ...
    def _run_pipeline(self):
        ...
        try:
            ...
            try:
                self._download_asset(ASSET_EXE, str(new_exe), start_pct=2, end_pct=35)
            except urllib.error.HTTPError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_EXE}: HTTP {e.code} {e.reason}"
                )
            except urllib.error.URLError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_EXE}: {e.reason}\n\n"
            ...
            try:
                self._download_asset(ASSET_UNINST, str(new_uninst), start_pct=35, end_pct=58)
            except urllib.error.HTTPError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_UNINST}: HTTP {e.code} {e.reason}"
                )
            except urllib.error.URLError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_UNINST}: {e.reason}\n\n"
                    "Check your internet connection."
                )
            ...
            try:
                self._download_asset(ASSET_BIN, str(zip_path), start_pct=58, end_pct=82)
            except urllib.error.HTTPError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_BIN}: HTTP {e.code} {e.reason}"
                )
            except urllib.error.URLError as e:
                raise RuntimeError(
                    f"Failed to download {ASSET_BIN}: {e.reason}\n\n"
                    "Check your internet connection."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"The downloaded {ASSET_BIN} archive is corrupted or invalid."
                )
            ...
            try:
                aurora_old.unlink(missing_ok=True)
            except OSError:
                pass
            ...
            try:
                uninst_old.unlink(missing_ok=True)
            except OSError:
                pass
            if uninst_live.exists():
                uninst_live.rename(uninst_old)
            new_uninst.rename(uninst_live)

            if bin_live.exists():
                shutil.rmtree(bin_live)
