...
def save_config(game_path):
    try:
        with open(CONFIG_FILE, "w", encoding="utf-8") as f: json.dump({"game_path": str(game_path)}, f)
    except Exception as e: pass

def load_config():
    if not os.path.exists(CONFIG_FILE): return None
    try:
        if os.path.getsize(CONFIG_FILE) == 0: return None
        with open(CONFIG_FILE, "r", encoding="utf-8") as f: return json.load(f).get("game_path")
    except (json.JSONDecodeError, AttributeError, OSError): return None
...
def get_game_directory():
    ...
    if saved_path:
        if validate_path(saved_path):
            ...
            return saved_path
        else: _warn(f"Saved config path is no longer valid: {saved_path}")
